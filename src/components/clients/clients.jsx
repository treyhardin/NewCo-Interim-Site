            ...
            <div class={styles.clientCardRow} ref={scrollerRow[0]}>
                <For each={data()}>{(client, i) =>
                    ...
                    <div class={styles.clientCard} key={i()}>
                        <img width="33vw" height="30vh" class={styles.clientImage} src={urlFor(client.image).width(800)} />
                        <img class={styles.clientLogo} src={urlFor(client.logo).width(500)} />
                    </div>
                    </Show>
                }</For>
            </div>
            ...
            <div class={styles.clientCardRow} ref={scrollerRow[1]}>
                <For each={data()}>{(client, i) =>
                    <Show when={i() > data().length / 3 && i() <= data().length / 3 * 2}>
                        <div class={styles.clientCard} key={i()}>
                            <img width="500" height="300" class={styles.clientImage} src={urlFor(client.image).width(800)} loading="eager" alt={`Lifestyle image for ${client.name}`} />
            ...
            <div class={styles.clientCardRow} ref={scrollerRow[2]}>
                <For each={data()}>{(client, i) =>
                    <Show when={i() > data().length / 3 * 2}>
                        <div class={styles.clientCard} key={i()}>
                            <img width="500" height="300" class={styles.clientImage} src={urlFor(client.image).width(1200)} loading="eager" alt={`Lifestyle image for ${client.name}`} />
                            <img width="500" height="500" class={styles.clientLogo} src={urlFor(client.logo).width(1000)} loading="eager" alt={`Logo for ${client.name}`} />
                        </div>
                    </Show>
                }</For>
            </div>
